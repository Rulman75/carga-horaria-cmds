...
async function main() {
  ...
  const encData = XLSX.utils.sheet_to_json<any>(encWorkbook.Sheets[encWorkbook.SheetNames[0]]);

  console.log('Leyendo DET...');
  const detWorkbook = XLSX.readFile(detFile);
  const detData = XLSX.utils.sheet_to_json<any>(detWorkbook.Sheets[detWorkbook.SheetNames[0]]);
